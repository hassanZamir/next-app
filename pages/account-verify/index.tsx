...
const AccountVerify: NextPage<{}> = () => {
    ...
    return <div className="d-flex align-items-center flex-column">
        ...
        <div style={{ margin: "200px 0px" }} className="d-flex flex-column align-content-center justify-content-center">
            {loading && <h3 className="mt-5">Loading...</h3>}
            {!loading && <h5 className={ message.type === 'success' ? 'text-success' : 'text-danger'}>{ message.text }</h5>}
            {!loading && message.type === 'success' && <Link href="/">Login</Link>}
        </div>
    </div>;
};
