...
const account = "venodev"; // TODO - Convert to env
const accountKey =
    "BvPGmzmGeRuJgEXcHvQn+HCI+iuYA5wY8eUhTt5B2vMlODMySKnezgcBRqNcM7x3e9rLIs0UaauMx1HZI8doow=="; // TODO: Convert to env
...
const storage = multer.diskStorage({
    ...
    filename: function (req: any, file: any, cb: any) {
        cb(
            null,
            Date.now() + "##" + (file.originalname ? file.originalname : "")
        );
    },
});
const upload = multer({
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (blurImage && blobBlurName) {
                const blurBlockBlobClient = containerClient.getBlockBlobClient(
                    blobBlurName
                );
            ...
            if (blobBlurName && blurImage)
                resolve({
                    ...
                    thumbnailUrl: blobBlurName,
                });
            else resolve({ url: blobName, type: isVideo ? 2 : 1 });
        } catch (err) {
            console.log("ERR : ", err);
...
const resizeImage = (file: any, width: any, height: any) => {
    return new Promise((resolve, reject) => {
        sharp(file.path)
            ...
            .then((data: any) => {
                const newFilePath =
                    "uploads/resize/" + file.path.split("uploads/")[1];
...
const blurImage = (file: any) => {
    return new Promise((resolve, reject) => {
        sharp({
            ...
            .then((data: any) => {
                const newFilePath =
                    "uploads/blur/" + file.path.split("uploads/")[1];
                fs.writeFile(newFilePath, data, function (err: any) {
    ...
    upload(req, res, function (err: any) {
                ...
                if (shouldBlur) {
                    resizeImage(file, dimensions[0][0], dimensions[0][1])
                        .then(function (resizedFile: any) {
                            blurImage(file).then(function (blurImage: any) {
        ...
        function sendResponse() {
            if (uploadPaths.length + failedPaths.length >= req.files.length)
                res.end(
                    JSON.stringify({
                        status: failedPaths.length <= 0 ? true : false,
                        uploadSuccess: uploadPaths,
                        uploadError: failedPaths,
                    })
                );
        }
    });
