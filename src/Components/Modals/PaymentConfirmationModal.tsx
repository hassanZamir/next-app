...
declare namespace IPaymentConfirmationModal {
    export interface IProps {
        ...
        onFollowConfirm: (a: boolean)=>void;
        paymentMode: number;
        creatorProfile: CREATOR_PROFILE
    }
}

export const PaymentConfirmationModal: React.RefForwardingComponent<HTMLDivElement, IPaymentConfirmationModal.IProps> = ((props) => {
    const { isShowing, modalRef, toggle, profileUserName, onFollowConfirm, paymentMode, creatorProfile } = props;
            ...
            <Modal border={theme.colors.primary} borderRadius="18px" width="330px">
                ...
                {paymentMode === 1 && <React.Fragment>
                    ...
                    <div className="my-5 px-4 d-flex justify-content-between">
                        <div onClick={()=>{ onFollowConfirm(true); toggle(); }} className="cursor-pointer px-3 px-1 bg-primary text-white rounded d-flex align-items-center justify-content-center">Yes</div>
                ...
                {paymentMode === 2 && <React.Fragment>
                    <div className="text-grey100 font-12px text-center my-5">
                        Are you sure you want to make a payment to subscribe 
                        <span className="seoge-ui-bold">{' ' + profileUserName + ' '}</span> from your wallet
                        
                    </div>
                    <div className="position-relative d-flex flex-column align-items-center justify-content-center">
                        <div className="position-absolute background-circle-primary" 
                            style={{ height: "80px", width: "80px" }}></div>
                        <div className="font-20px text-primary lato-bold">
                            Pay ${creatorProfile.followingFee}
                        </div>
                        <div style={{ width: "100px", height: "2px" }} className="bg-primary"></div>
                    </div>
                    <div className="my-5 px-4 d-flex justify-content-between">
                        <div onClick={()=>{ onFollowConfirm(true); toggle(); }} className="cursor-pointer px-3 px-1 bg-primary text-white rounded d-flex align-items-center justify-content-center">Yes</div>
                        <div onClick={()=>{ toggle() }} className="cursor-pointer px-3 py-1 bg-primary text-white rounded d-flex align-items-center justify-content-center">No</div>
