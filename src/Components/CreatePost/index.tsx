...
export const CreatePost: React.FunctionComponent<{ user: USER_SESSION; }> 
    ...
    const savePost = async () => {
        ...
        setLoading(true);
        await dispatch(FeedsActions.PostContent({ 
    ...
    return <div style={{ minHeight: "135px" }} className="my-3 d-flex flex-column flex-fil border-top border-bottom border-primary">
        {files.length > 0 && <div className="px-2 py-1 d-flex align-items-center">
            {files.map((url, i) => {
                ...
                return (<React.Fragment key={i}>
                    {url.preview && !isVideo && <img src={url.preview} width="38" height="36" />}
                    {url.preview && isVideo && <video width="38" height="36" controls={false}>
