            ...
            <div className="d-flex flex-column pl-2 w-100 justify-content-between">
                <div className="d-flex justify-content-between align-items-center w-100">
                    <Link href={`/profile/${comment.userName}`}>
                        <a className="seoge-ui-bold font-13px text-primary cursor-pointer">
                            {comment.userName}
                        </a>
                    </Link>
                    <div className="d-flex align-items-center">
                        <div
                            className="d-flex align-items-center cursor-pointer mr-1"
                            onClick={() => {
                                likeComment(comment);
                            }}
                        >
                            <span className="font-13px text-darkGrey mr-1">
                                {comment.likesCount || 0}
                            </span>
                            <FontAwesomeIcon
                                icon={faHeart}
                                color={
                                    comment.content_viewer_like
                                        ? "#F57B52"
                                        : "#A0A0A0"
                                }
                                size="lg"
                ...
                <div className="font-11px text-darkGrey">
                    {CurrentTimeDifference(comment.timeStamp)}
                </div>
            </div>
        </div>
    );
...
const CommentsList: React.FunctionComponent<{
    onSeeLatestComments: () => void;
    scrolledToTop: boolean;
    commentsCount: number;
    onScroll: any;
    viewAllComments: boolean;
    setViewAllComments: (a: boolean) => void;
    ...
    onScroll,
        ...
        const [defaultCommentsCount, setDefaultCommentCount] = useState(
            commentsCount
        );
        ...
        return (
            ...
            >
                ...
                {!viewAllComments && !loading && commentsCount > 6 && scrollingTop && (
                    <div
                        onClick={() => {
                            setViewAllComments(true);
                            setShowSeeAllComments(false);
                            onViewAllComments();
                        }}
                ...
                {viewAllComments && !loading && showSeeAllComments && (
                    <div
                        onClick={() => {
...
const PostComment: React.FunctionComponent<{
    user: USER_SESSION;
    contentId: number;
    error: string;
    onSuccess: () => void;
            ...
            <div className="d-flex align-items-center justify-content-between">
                ...
                <div className="d-flex justify-content-between align-items-center w-100 ml-2">
                    ...
                    <div
                        ...
                        className={
                            "cursor-pointer ml-2 rounded-circle d-flex align-items-center justify-content-center " +
                            (loading || comment === ""
                                ? "bg-darkGrey"
                                : "bg-primary")
                        }
...
export const Comments: React.FunctionComponent<{
    ...
    const getComments = async (
        params: IStatusPage.Actions.IGetAllCommentsPayload
    ) => {
    ...
    return (
        <div className="position-relative full-flex-scroll d-flex flex-column w-100 h-100">
            <CommentsList
